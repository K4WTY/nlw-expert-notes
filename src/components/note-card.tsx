...
interface NoteCardProps { // Declarando as propriedades do NoteCard
    note: {
        id: string
        data: Date
        content: string
...
export function NoteCard({ note, onNoteDeleted }: NoteCardProps) { // (props: NoteCardProps) informando as propriedades utilizando a interface
    return (
        <Dialog.Root>
            <Dialog.Trigger className="rounded-md text-left flex flex-col bg-slate-800 p-5 gap-3 outline-none overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400">
                <span className="text-sm font-medium text-slate-300"> { formatDistanceToNow(note.data, { locale: ptBR, addSuffix: true }) } </span>
